package techkids.mad3.learninternalexternalstorage;

/**
 * Created by TrungNT on 5/27/2016.
 */
public class Helper {
    public static String fileName = "MyAccountPrivate"; //ten file luu Email va Password
    public static String alertDownload = "Download successfully your flle"; //alert donload
    public static String fileNameDownload = "download";
    public static final String EXTERNAL_STORAGE_FILE_NAME = "QuocCa-HoaTau_xcj4.mp3";
    public static final String INTERNAL_STORAGE_FILE_NAME = "py.png";
}
